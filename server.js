...
async function topThirty() {
  ...
  const ranked = await mapLimit(candidates, 12, async item => { const rows1m = await api(`/fapi/v1/klines?symbol=${item.symbol}USDT&interval=1m&limit=60`), liveVolume = rows1m.slice(-16, -1).reduce((sum, row) => sum + (+row[7] || 0), 0); return { ...item, rows1m, liveVolume }; });
  return ranked.sort((a, b) => b.liveVolume - a.liveVolume).slice(0, 30);
}

async function findHundredPointSurges(top30) {
  const analyses = await mapLimit(top30, 10, async item => { const rows5m = await api(`/fapi/v1/klines?symbol=${item.symbol}USDT&interval=5m&limit=60`), one = score(item.rows1m), five = score(rows5m); if (!one || !five || one.signal !== "롱" || five.signal !== "롱" || one.confidence !== 100 || five.confidence !== 100) return null; return { symbol: item.symbol, current: one.current, score1m: one.confidence, score5m: five.confidence, executionStrength: one.executionStrength, change1m: one.change1m, liveVolume: item.liveVolume }; });
